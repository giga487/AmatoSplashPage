...
namespace Components.Skia
{
    public class SkiaBarDrawable : SkiaObjectBase
    {
        ...
        public float CurrentY => Position == BarPosition.Top
            ? 0f
            : _scene.CanvasHeight - BarHeight;

        public SkiaBarDrawable(SkiaScene scene, BarPosition position, float barHeight, SKColor color)
        {
            _scene = scene;
            Position = position;
            BarHeight = barHeight;
